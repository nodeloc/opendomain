<template>
  <div class="min-h-[70vh] flex items-center justify-center px-4 sm:px-6 py-8">
    <div class="card w-full max-w-lg shadow-2xl bg-base-200">
      <div class="card-body">
        <h2 class="card-title text-3xl font-bold justify-center mb-6">{{ $t('register.title') }}</h2>
        ...
        <template v-else>
          ...
          <template v-if="siteConfig.allow_password_register">
            <form @submit.prevent="handleRegister" class="space-y-4">
              <div class="form-control">
                <label class="label">
                  <span class="label-text">{{ $t('register.username') }}</span>
                </label>
                <input
              ...
              <div class="form-control">
                <label class="label">
                  <span class="label-text">{{ $t('register.inviteCode') }}</span>
                </label>
                <input
              ...
              <div class="form-control mt-6">
                <button type="submit" class="btn btn-primary" :disabled="loading">
                  ...
                  <span v-else>{{ $t('register.signUpButton') }}</span>
                </button>
              </div>
            </form>
          ...
          <div v-else class="alert alert-warning mb-4">
            ...
            <span>{{ $t('register.registrationDisabled') }}</span>
          </div>

          <!-- OAuth buttons (only show configured ones) -->
          ...
          <div class="text-center mt-4">
            <p class="text-sm">
              ...
              <router-link to="/login" class="link link-primary font-semibold">{{ $t('register.login') }}</router-link>
            </p>
          </div>
        </template>
      </div>
...
const siteConfig = computed(() => ({
  allow_password_register: siteConfigStore.allowPasswordRegister,
...
const handleRegister = async () => {
  ...
  if (form.value.invite_code) {
    payload.invite_code = form.value.invite_code
  }
